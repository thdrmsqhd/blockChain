...
class Block {
    ...
    static validateStructure = (aBlock:Block) :boolean =>{//들어온 블록안의 내용물이 블록 형식에 맞는지 검증하는 함수
...
const getNewTimestamp = (): number => Math.round(new Date().getTime() / 100)//새로운 타임스탬프를 만드는 함수

const createNewBlock = (data:string):Block =>{//새블록을 만드는 함수
    const previousBlock :Block = getLatestBlock();//이전 블록 (블록체인의 마지막 블록을 가져온다.)을 가져온다.
    const newIndex: number = previousBlock.index + 1;//이전 블록 인덱스에 1을 더한 값
    const newTimestamp:number = getNewTimestamp();// 현재 시간을 기준으로 새로운 타임스탬프를 만든다.
    const newHash: string = Block.calculateBlockHash(newIndex,previousBlock.hash,newTimestamp,data);//해쉬 계산 함수를 이용해 새로운 해쉬를 만들어낸다.
    const newBlock: Block = new Block(newIndex, newHash, previousBlock.hash, data, newTimestamp);//매개변루를 이용해 새로운 블록을 생성한다.
    addBlock(newBlock);//정합성 검증 및 블록체인 리스트에 블록을 추가한다.
    return newBlock;
}

const getHashBlock = (aBlock:Block):string => {//Crypto.JS 에 값은 매개변수를 가지고 들어가면 같은 결과값이 나온다.
    return Block.calculateBlockHash(aBlock.index, aBlock.previousHash, aBlock.timestamp, aBlock.data)
}


const isBlockValid = (candidateBlock:Block, previousBlock:Block):boolean=>{//블록 구조가 맞는지 + 인덱스, 해쉬, 새로운 해쉬값을 체크하는 함수
...
const addBlock = (candidateBlock: Block):void=>{//새로운 블럭을 블록체인 리스트에 추가하는 함수
